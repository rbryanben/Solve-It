...
public class confirmReplyReplyDeletion {
    Activity activity ;
    AlertDialog dialog ;
    String QUESTION_ID ;
    ...
    public confirmReplyReplyDeletion(Activity activity, String QUESTION_ID, repliesGridClickInterface in) {
                ...
                mDatabase.child("deleted_replies").push().setValue(QUESTION_ID).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                         if (task.isSuccessful()){
                              //delete the question
                              mDatabase.child("repliesToReply").child(QUESTION_ID).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                  @Override
                                  public void onComplete(@NonNull Task<Void> task) {
                                       if (task.isSuccessful()){
                                           stopAlert();
