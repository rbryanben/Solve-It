...
public class confirmDeletion {
    ...
    public confirmDeletion(Activity activity, String QUESTION_ID, homeQuestionsInterface in) {
    ...
    public void startAlert(){
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //register deletion
                mDatabase.child("deleted_questions").push().setValue(QUESTION_ID).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                         if (task.isSuccessful()){
                              //delete the question
                              mDatabase.child("questions").child(QUESTION_ID).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                  @Override
                                  public void onComplete(@NonNull Task<Void> task) {
                                       if (task.isSuccessful()){
                                           stopAlert();
                                           update.update();
                                           ((globalShared) activity.getApplication()).showToast(activity,"We deleted your question");
                                       }
                                       else {
